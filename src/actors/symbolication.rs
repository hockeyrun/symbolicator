        ...
        let mut inner: Vec<(CompleteObjectInfo, bool)> = minidump_process_state
            ...
            .map(|code_module| {
                ...
                if let Some(code_id) = code_module.id() {
                    if let Some(cfi_module) = cfi_caches.get(&code_id) {
                        obj_info.unwind_status = cfi_module.cfi_status;
                ...
                metric!(
                    ...
                    "status" => obj_info.unwind_status.name(),
                );
...
impl SymbolicationActor {
    ...
    async fn stackwalk_minidump_with_cfi(
        ...
        let lazy = async move {
            let spawn_time = std::time::SystemTime::now();
            let spawn_result = pool.spawn(
                (cfi_caches, minidump.clone(), spawn_time),
                |(mut cfi_caches, minidump, spawn_time)| -> Result<_, ProcessMinidumpError> {
                    if let Ok(duration) = spawn_time.elapsed() {
